...
#define STR(s) #s
Mat textDetection (const Mat& input, bool dark_on_light,std::string name) {
   ...
   // assert ( input.channels() == 3 );
    ...
    tool::DebugOut(STR(edgeImage), name, edgeImage);
    ...
    tool::DebugOut(STR(SWTImage), name, SWTImage);
    SWTMedianFilter ( SWTImage, rays );
    tool::DebugOut(STR(SWTImage1), name, SWTImage);
    Mat output2( input.size(), CV_32FC1 );
    normalizeImage (SWTImage, output2);
    tool::DebugOut(STR(output2), name, output2);
    Mat saveSWT( input.size(), CV_8UC1 );
    output2.convertTo(saveSWT, CV_8UC1, 255);

    tool::DebugOut(STR(saveSWT), name, saveSWT);
    ...
    tool::DebugOut(STR(output3), name, output3);
    //
